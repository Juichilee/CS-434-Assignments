...
for m in range(M):
  ...
  # random_data = np.empty((0, 30))
  # random_labels = np.empty((0))
  
  # while random_data.shape[0] < X_train.shape[0]:
  #   idx = rand.randrange(X_train.shape[0])
  #   chosen_arr = np.expand_dims(X_train[idx], 0)
  #   random_data = np.append(random_data, chosen_arr, axis=0)
  #   random_labels = np.append(random_labels, y_train[idx])
  
  # X_data = random_data
  # y_data = random_labels
  X_data = X_train
  y_data = y_train

  #Fit the model and store it's predictions
  clf = tree.DecisionTreeClassifier("entropy", max_features=15)
